...
import word2vec
from model_multilabel import FastTextMultiLabel as FastText
from p4_zhihu_load_data import (creat_vocabulary, creat_vocabulary_label,
                                load_data_with_multilabels)
...
def main():
    ...
    train, test, _ = load_data_with_multilabels(
        vocabulary_word2index, vocabulary_word2index_label, FLAGS.training_path)
    trainX, trainY = train
    testX, testY = test
    ...
    with tf.Session(config=config) as sess:
        ...
        if os.path.exists(FLAGS.ckpt_dir + "checkpoint"):
            ...
            if FLAGS.use_embedding:  # 是否使用预训练的word2vec词向量
...
def get_label_using_logits(logits, vocabulary_index2word, top_n=5):
    index_list = np.argsort(logits)[-top_n:]
    index_list = index_list[::-1]
    return index_list


def calculate_acc(labels_pre, labels, eval_cnt):
    if eval_cnt < 10:
        print("labels_pre:", labels_pre, " ;labels", labels)
    cnt = 0.
    label_dict = {x: x for x in labels}
    for label_pre in labels_pre:
        flag = label_dict.get(label_pre, None)
    if flag is not None:
        cnt += 1.
