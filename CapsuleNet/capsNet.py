...
class CapsNet(object):
    ...
    def build_model(self):
        ...
        with tf.variable_scope('DigitCaps_layer'):
            digiCaps = CapsLayer(num_outputs=10,
                                 vec_len=16,
                                 with_routing=True,
                                 layer_type='FC')
            self.caps2 = digiCaps(self.caps1)
    ...
    def _summary(self):
        ...
        correct_pre = tf.equal(tf.to_int32(self.labels), self.argmax_idx)
        self.batch_acc = tf.reduce_mean(tf.cast(correct_pre, tf.float32))
