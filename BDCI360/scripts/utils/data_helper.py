...
class ArticleSample(object):
    def __init__(self, id, title, deal_title, content, deal_content, judge,deal_judge, cv):
        """
        文章对象
        :param id:  文章ID(str)
...
def build_vocab(file_path, voc_path):
    ...
    voc_dict = {'voc':voc, 'max_title_length':max_title_length, 'max_content_length':max_content_length}
...
def load_data_cv(file_path, voc_path, mode, cv=5):
    ...
    if os.path.isfile(voc_path):
        ...
        voc, max_title_length, max_content_length = build_vocab(file_path, voc_path)
    ...
    with open(file_path, encoding='utf-8') as tsvfile:
        ...
        for line in tsvreader:
            ...
            title, content = title[:max_title_length], content[:max_content_length]
            ...
            deal_judge = 1 if judge == 'POSITIVE' else 0
            article = ArticleSample(id=id, title=title, deal_title=deal_title,
                                    content=content, deal_content=deal_content,
                                    judge=judge, deal_judge =deal_judge, cv=cv)
            rev.append(article)
